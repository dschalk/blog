...
{/if}
<br>
<p> The cult that captivated me has no makeshift control towers or dirt runways made to entice cargo-bearing planes to land. It was authors, presenters, and bloggers who mimic      superficial aspects of functional languages such as Haskell  thinking, without understanding, that it improves their code.  it improves their code don't mutate variables or alter the contents of objects. Their functions are strictly typed, referentially transparent, and take no more than one argument. </p>

<p> Writing this kind of JavaScript code won't entitle you to cult membership if you do it for amusement, experimentation, or whatever satisfaction it brings Maybe it was my enthusiasm for the Haskell programming language that made it so easy for me to hope that mimicking Haskell's essential characteristics would result in better Javascript code. </p>

<p> I constrained my JavaScript code in other ways too, favoring "good parts" over "bad parts", and "best practices" over common sense. After a while it dawned on me that when the global space is a little module in an application, global variables are not dangerous. When a million and one non-tail-call recursions or passes through a loop can either spew a million pieces of garbage into memory, or else mutate something a million times, there should be a good reason for choosing to feed the garbage collector such a large feast. That's when I unleashed the full power of JavaScript and started having fun.  </p>
